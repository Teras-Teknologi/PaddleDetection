...
from PaddleDetection.deploy.pipeline.pipe_utils import (
    get_test_images,
    crop_image_with_det,
    crop_image_with_mot,
    parse_mot_res,
...
from PaddleDetection.deploy.pptracking.python.mot.visualize import plot_tracking_dict
from PaddleDetection.deploy.pptracking.python.mot.utils import (
    flow_statistic,
    update_object_info,
)
    ...
    def predict_video(self, video_file, camera_id):
        ...
        while 1:
            ...
            im = visualize_box_mask(
                frame, results, self.pred_config.labels, threshold=self.threshold
            )
            im = np.array(im)
            writer.write(im)
            if camera_id != -1:
            ...
            if "masks" in results:
                ...
                for box, mask in zip(boxes, masks):
                    rle = mask_util.encode(
                        np.array(mask[:, :, None], dtype=np.uint8, order="F")
                    )[0]
                    if "counts" in rle:
...
def load_predictor(
    ...
    if run_mode in precision_map.keys():
        ...
        if FLAGS.collect_trt_shape_info:
            ...
            print(
                f"Use dynamic shape file: " f"{FLAGS.tuned_trt_shape_file} for TRT..."
            )
...
def visualize(image_list, result, labels, output_dir="output/", threshold=0.5):
    ...
    for idx, image_file in enumerate(image_list):
        ...
        im_results = {}
        if "boxes" in result:
            im_results["boxes"] = result["boxes"][
                start_idx : start_idx + im_bboxes_num, :
            ]
        if "masks" in result:
            im_results["masks"] = result["masks"][
                start_idx : start_idx + im_bboxes_num, :
            ]
        if "segm" in result:
            im_results["segm"] = result["segm"][
                start_idx : start_idx + im_bboxes_num, :
            ]
        if "label" in result:
